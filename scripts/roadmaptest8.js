...
function createProjectgantt(x, y, z) {

   var projectJson = x; //Json data
   var projectCount = y; //Amount of Projects.
   var projectsToDisplay = 0; //requested amount of projects
   ...
   var activeProjects = []; //Store active projects.
   var activeMembers = []; //Store active Members on projects.
   var shownMembers = []; //Array for members currently supposed to be displayed.
   var chartData = []; //Data used to build chart.
   var chartStructure = []; //Data for chart structure.
   var endDateArray = []; //End dates for active projects
   ...
   projectsToDisplay = z; //grab z value if it exists.
   ...
   /*       //Set end date
        ...
        //Find the earliest date by comparing the date from the current Project to our current earliest date.
...
*/
   ...
   while (counterProject < projectCount) {
      ...
      if (projectJson.projects[counterProject].start && projectJson.projects[counterProject].end) {
         //check if member is supposed to be shown.
      ...
      /**If a project has a member
       *that is supposed to be shown,
       *push into activeProjects array.
       */
      if (displayedProj === 1) {
   ...
   /* $('div.arrows a.doubleLeft').click(function () {
       return false;
    });
    $('div.arrows a.Left').click(function () {
       index++;
       update();
       return false;
    });
    $('div.arrows a.Right').click(function () {
       index--;
       update();
       return false;
    });
    $('div.arrows a.doubleRight').click(function () {
       return false;
    });*/

   if (activeProjects.length >= projectsToDisplay + index) {
      if (activeProjects.length < projectsToDisplay + index + 5) {
         //double left arrow not clickable
         $('div.arrows a.doubleLeft').addClass("unclickable");
         if (activeProjects.length < projectsToDisplay + index + 1) {
            //left arrow not clickable
            $('div.arrows a.Left').addClass("unclickable");
         } else {
            //left arrow clickable
            $('div.arrows a.Left').removeClass("unclickable");
         }
      } else {
         //double left arrow is clickable
         $('div.arrows a.doubleLeft').removeClass("unclickable");
         //left is clickable
         $('div.arrows a.Left').removeClass("unclickable");
      }
      activeProjects = activeProjects.slice(index, index + projectsToDisplay);
      // createArrows();
   } else if (activeProjects.length >= projectsToDisplay) {
      index = activeProjects.length - projectsToDisplay;
      if (activeProjects.length < projectsToDisplay + index + 5) {
         //double left arrow not clickable
         $('div.arrows a.doubleLeft').addClass("unclickable");
         if (activeProjects.length < projectsToDisplay + index + 1) {
      ...
      activeProjects = activeProjects.slice(activeProjects.length - projectsToDisplay, activeProjects.length);
      //cut off end, let activeProjects get max size, remove projects + index
      //createArrows();
      //alert(activeProjects.length + " " + projectsToDisplay);
      //alert(index);
   } else {
      index = 0;
      if (activeProjects.length < projectsToDisplay + index + 5) {
   ...
   if (index < 5) {
      //double right is unclickable
      $('div.arrows a.doubleRight').addClass("unclickable");
      if (index == 0) {
         //right is unclickable
         $('div.arrows a.Right').addClass("unclickable");
      } else {
         //right is clickable
         $('div.arrows a.Right').removeClass("unclickable");
      }
   } else {
      //right is clickable
      $('div.arrows a.Right').removeClass("unclickable");
      //double right is clickable
      $('div.arrows a.doubleRight').removeClass("unclickable");
   }
   
   $('div.arrows span').html(index);

   //Reset our counter.
   counterProject = 0;
console.log(activeMembers);
   //Use our now complete list of
   while (counterProject < activeProjects.length) {

      //check if leader is listed in activeMember array, if not push them into the array.
      if (activeProjects[counterProject].leader && contains(shownMembers, activeProjects[counterProject].leader) && !contains(activeMembers, activeProjects[counterProject].leader)) {
         activeMembers.push(activeProjects[counterProject].leader);
         console.log(activeProjects[counterProject].leader);
         console.log(activeProjects[counterProject]);
      ...
      for (i = 1; i < 7; i++) {
         ...
         if (contains(shownMembers, activeProjects[counterProject][memberNumber]) && !contains(activeMembers, activeProjects[counterProject][memberNumber]) && activeProjects[counterProject][memberNumber]) {
            activeMembers.push(activeProjects[counterProject][memberNumber]);
            console.log(activeMembers);
         }
      }
      counterProject++;
   ...
   /**
    *Change chart heigh based
    *upon number of projects
    *being displayed and number
    *of employees
    */
...
function findLastDate(x) {
   /*Find and return the latest date for project 
   x based on completion, hold, extension, or 
   current day.*/
   var dateVar = Date.parse(x.end);

   if (x.extensionDate && Date.parse(x.extensionDate) > dateVar) {
      dateVar = Date.parse(x.extensionDate);
   }

   if (x.holdEnd && Date.parse(x.holdEnd) > dateVar) {
      dateVar = Date.parse(x.holdEnd);
   }

   if (x.completionDate) {
      if (Date.parse(x.completionDate) > dateVar) {
         dateVar = Date.parse(x.completionDate);
      }
   } else {
...
function orderData(x, y) {
   ...
   while (counterProject < projectCount) {
      currentProj = theJsonData.projects[counterProject];

      //Get the last date for project.
      dateVar = findLastDate(currentProj);
      console.log(dateVar);
      if (activeProjects.length === 0) {
         endDateArray.push(dateVar);
         activeProjects.push(currentProj);
      } else {
         i = 0;
         while (i < counterProject && activeProjects.length < counterProject + 1) {
            if (Date.compare(dateVar, endDateArray[i]) == 1) {
               endDateArray.splice(i, 0, dateVar);
               activeProjects.splice(i, 0, currentProj);
            } else {
   ...
   theJsonData.projects = activeProjects;
   return theJsonData;
}
...
function update() {
   createProjectgantt(jsonData.projectJson, jsonData.projectCount, parseInt($('select.projectCount').val(), 10));
}
...
$(document).ready(function () {
   ...
   //Order our data by end date. 
   jsonData.projectJson = orderData(jsonData.projectJson, jsonData.projectCount);
